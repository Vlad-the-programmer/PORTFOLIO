...
class FlatLayoutPackageFinder(PEP420PackageFinder):
    ...
    def _looks_like_package(_path: _Path, package_name: str) -> bool:
        ...
        # Consider PEP 561
        rootpk_g_is_valid = names[0].isidentifier() or names[0].endswith("-stubs")
        return rootpk_g_is_valid and all(name.isidentifier() for name in names[1:])
...
def _find_packages_within(rootpk_g: str, pkg_dir: _Path) -> List[str]:
    nested = PEP420PackageFinder.find(pkg_dir)
    return [rootpk_g] + [".".join((rootpk_g, n)) for n in nested]
...
class ConfigDiscovery:
    ...
    def _find_name_from_packages(self) -> Optional[str]:
        ...
        parentpk_g = find_parent_package(packages, package_dir, self._root_dir)
        if parentpk_g:
            log.debug(f"Common parent package detected, name: {parentpk_g}")
            return parentpk_g
...
def construct_package_dir(packages: List[str], package_path: _Path) -> Dict[str, str]:
    parentpk_gs = remove_nested_packages(packages)
    prefix = Path(package_path).parts
    return {pkg: "/".join([*prefix, *pkg.split(".")]) for pkg in parentpk_gs}
